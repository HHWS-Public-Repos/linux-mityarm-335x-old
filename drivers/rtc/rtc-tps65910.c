...
#else
	ret = tps->read(tps, TPS65910_RTC_CTRL, 1, &tmp);
	tmp |= TPS65910_RTC_CTRL_GET_TIME;
	ret = tps->write(tps, TPS65910_RTC_CTRL, 1, &tmp);
#endif
	if (ret < 0) {
...
#else
	ret = tps->read(tps, TPS65910_RTC_CTRL, 1, &tmp);
	tmp &= ~TPS65910_RTC_CTRL_STOP_RTC;
	ret = tps->write(tps, TPS65910_RTC_CTRL, 1, &tmp);
#endif
	if (ret < 0) {
...
#else
	ret = tps->read(tps, TPS65910_RTC_INTERRUPTS, 1, &tmp);
	int_val = tmp;
#endif
	if (ret < 0)
		return ret;
...
#else
	unsigned char tmp;
	ret = tps->read(tps, TPS65910_RTC_STATUS, 1, &tmp);
	rtc_reg = tmp;
#endif
	if (ret)
		return IRQ_NONE;
...
#else
	tmp = rtc_reg;
	ret = tps->write(tps, TPS65910_RTC_STATUS, 1, &tmp);
#endif
	if (ret)
		return IRQ_NONE;
...
#else
	ret = tps65910->read(tps65910, TPS65910_RTC_STATUS, 1, &tmp);
	rtc_reg = tmp;
#endif
	if (ret < 0)
		return ret;
...
#else
	tmp = rtc_reg;
	ret = tps65910->write(tps65910, TPS65910_RTC_STATUS, 1, &tmp);
#endif
	if (ret < 0)
		return ret;
...
#else
	tmp = rtc_reg;
	ret = tps65910->write(tps65910, TPS65910_RTC_CTRL, 1, &tmp);
#endif
	if (ret < 0)
		return ret;
...
#else
	return tps->write(tps, TPS65910_RTC_INTERRUPTS, 1, &alarm);
#endif
}
